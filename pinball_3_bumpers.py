...
def setup_boundaries(space):
    ...
    # This collision handler has methods that are called when a collision between the two collision types specified
    # (in this case ball and boundary) occurs.
...
def add_bumper(space, position):
    bumper_body = pymunk.Body(body_type=pymunk.Body.STATIC)
    bumper_body.position = position

    bumper_shape = pymunk.Circle(body=bumper_body, radius=26)
    bumper_shape.collision_type = collision_types["bumper"]

    space.add(bumper_body, bumper_shape)


def setup_bumpers(space):
    add_bumper(space, (300, 450))
    add_bumper(space, (150, 250))
    add_bumper(space, (150, 650))
    add_bumper(space, (450, 250))
    add_bumper(space, (450, 650))

    # This collision handler has methods that are called when a collision between the two collision types specified
    # (in this case ball and bumper) occurs.
    ch = space.add_collision_handler(collision_types["ball"], collision_types["bumper"])

    def push_ball(arbiter, space, data):
        ball_shape = arbiter.shapes[0]
        bumper_shape = arbiter.shapes[1]

        # The direction is a unit vector pointing from the bumper's center to the ball's center
        direction = unit_vector_between(ball_shape.body.position, bumper_shape.body.position)
        strength = 750  # The strength of the push

        # The direction vector is scaled by the variable strength
        impulse = [direction[0] * strength, direction[1] * strength]

        # The ball is pushed directly away from the center of the bumper
        ball_shape.body.apply_impulse_at_world_point(impulse, bumper_shape.body.position)

    # Once the ball has collided with the bumper, an impulse is applied to the ball away from the bumper.
    ch.post_solve = push_ball
